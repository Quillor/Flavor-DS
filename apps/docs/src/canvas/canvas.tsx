/* ─────────────────────────────────────────────────────────────────────────────
   Canvas — design-review board for Flavor DS.
   PORT of the EthiGov reference implementation (canvas-maker skill,
   reference/implementation.tsx == ethigov-incident-manager/src/features/canvas/
   pages/canvas.tsx). The board's mechanics, structure and chrome are the
   reference's, unchanged. Only three things differ, each per the skill's adapter
   contract:
     1. imports — shadcn/lucide/router/sonner come from ./shims.tsx on Flavor's own
        components and sprite icons; Tailwind utilities from ./canvas-tw.css;
     2. adapter data (routes, tokens, palette, spacing) — from ./design-system.ts,
        derived from this project's nav and token build outputs;
     3. colour math — Flavor stores HEX, not shadcn HSL triplets, so parse/format
        swap while the picker UI stays the same (adapter §3c);
   plus one marked [extension §7]: repo-file persistence via the dev server.
   ───────────────────────────────────────────────────────────────────────────── */
import * as React from 'react';
import {
  Link, toast, cn,
  ArrowLeft, ChevronsLeft, ChevronsRight, ClipboardCopy, Eye, EyeOff, ExternalLink, Maximize2, MessageSquare, Minimize2, Minus, Moon, Palette, Plus, RotateCw, Scan, SlidersHorizontal, Smartphone, Sun, Trash2, X,
  Button, Badge, Input, Textarea, Select, SelectContent, SelectItem, SelectTrigger, SelectValue, Popover, PopoverContent, PopoverTrigger, Tabs, TabsContent, TabsList, TabsTrigger,
} from './shims';
import {
  GROUPS, TYPOGRAPHY_TOKENS, TYPOGRAPHY_CSS_PROPS, COLOR_TOKENS, TEXT_COLOR_KEYS, BACKGROUND_COLOR_KEYS, PALETTE_GROUPS, SPACING_TOKENS, spacingCss,
  AXES, AXIS_DEFAULTS, AXIS_LABELS, applyAxes, VERSION, FEEDBACK_ENDPOINT,
} from './design-system';
import type { CanvasGroup, CanvasPage as CanvasPageEntry } from './design-system';
import { DEFAULT_HIDDEN } from './design-system';

/* ── page registry: from design-system.ts (siteNav) ─────────────────────── */
const ALL_PATHS = GROUPS.flatMap((g) => g.pages.map((p) => p.path));
...
/* ── design-token lookups for the live "tweak" inspector ─────────────────
   Every option here resolves to real token-backed CSS (the same values the
   design system itself uses), so a tweak can never introduce an off-system
   value. Applied two ways: (1) live, as inline styles injected into the
   target element inside its own iframe, so the change previews immediately;
   (2) as the exact spec text embedded in copied feedback.                 */

// TYPOGRAPHY_TOKENS, TYPOGRAPHY_CSS_PROPS, COLOR_TOKENS, TEXT_COLOR_KEYS, BACKGROUND_COLOR_KEYS: design-system.ts

/* ── color math for the token color picker ─────────────────────────────────
   Tokens are stored the way the design system stores them: either a raw HSL
   triplet ("209 64% 13%") or an alias to a palette token ("var(--teal-600)").
   The picker converts between that triplet, HSV (for the saturation/value
   square), and hex — and can resolve an alias back to a triplet so the picker
   opens on the right color.                                                 */
...
/* Flavor stores colours as HEX (and semantic tokens as var() aliases into a ramp).
   The reference stored shadcn HSL triplets. Same picker, different parse/format:
   parse hex | rgb() | HSL-triplet → Hsl; format Hsl → hex. */
function parseHslTriplet(value: string): Hsl | null {
  const v = value.trim();
  const hx = v.match(/^#([0-9a-f]{3}|[0-9a-f]{6})$/i);
  if (hx) { const h = hx[1].length === 3 ? [...hx[1]].map((c) => c + c).join('') : hx[1]; const n = parseInt(h, 16); return rgbToHsl((n >> 16) & 255, (n >> 8) & 255, n & 255); }
...
function rgbToHsl(r: number, g: number, b: number): Hsl {
  r /= 255; g /= 255; b /= 255; const mx = Math.max(r, g, b), mn = Math.min(r, g, b); const l = (mx + mn) / 2; let h = 0, s = 0;
  if (mx !== mn) { const d = mx - mn; s = l > 0.5 ? d / (2 - mx - mn) : d / (mx + mn); h = mx === r ? ((g - b) / d + (g < b ? 6 : 0)) : mx === g ? (b - r) / d + 2 : (r - g) / d + 4; h *= 60; }
  return { h: Math.round(h), s: Math.round(s * 100), l: Math.round(l * 100) };
}
/** "format" writes back in THIS project's stored format: hex. */
function formatHslTriplet(hsl: Hsl): string { return hslToHex(hsl); }
...
// Resolves a stored token value (override or the CSS default) down to a real
// HSL triplet, following one level of `var(--palette)` aliasing so the picker
// can seed itself even when the token points at a palette color.
function resolveTripletForVar(value: string | undefined, cssVar: string): Hsl {
  const root = document.documentElement;
  // getComputedStyle already follows var() alias chains to a concrete value
  ...
  return parseHslTriplet(raw) ?? { h: 0, s: 0, l: 0 };
}

// PALETTE_GROUPS, SPACING_TOKENS: design-system.ts (real primitives, real scale)
const SPACING_SIDE_PROPS = {
  padding: ['padding-top', 'padding-right', 'padding-bottom', 'padding-left'],
  margin: ['margin-top', 'margin-right', 'margin-bottom', 'margin-left'],
};
const SPACING_CSS_PROPS = [...SPACING_SIDE_PROPS.padding, ...SPACING_SIDE_PROPS.margin, 'gap'];

const LAYOUT_CSS_PROPS = ['display', 'flex-direction', 'flex-wrap', 'justify-content', 'align-items'];
...
const NONE = '__none__';
const TOKEN_STORAGE_KEY = 'canvas-token-overrides';
...
const FEEDBACK_PREFIX =
  "You're applying UI feedback collected on the Flavor DS docs /canvas board. For each numbered " +
...
function tweakSpecLines(tweak: AnnotationTweak | undefined): string[] {
  ...
  const spacingLines = (label: string, sides: SpacingSides | undefined) => {
    ...
    (['top', 'right', 'bottom', 'left'] as const).forEach((side) => {
      ...
      if (key && SPACING_TOKENS[key]) {
        lines.push(`tweak: ${label}-${side} → --${key} (${SPACING_TOKENS[key].rem * 16}px at regular density)`);
      }
    });
  };
  ...
  if (tweak.spacing?.gap && SPACING_TOKENS[tweak.spacing.gap]) {
    lines.push(`tweak: gap → --${tweak.spacing.gap} (${SPACING_TOKENS[tweak.spacing.gap].rem * 16}px at regular density)`);
  }
...
export function CanvasPage() {
  // adapter §1: the 47 component reference pages start hidden (they are a real group,
  // but 78 live frames at once opens the board at 5%); Core UI › eye toggles them on.
  const [hidden, setHidden] = React.useState<Set<string>>(() => new Set(DEFAULT_HIDDEN));
  ...
  // Flavor's theming bridge: light/dark is data-mode on <html>; seed from the site's saved
  // theme (or the OS) so opening Canvas from a dark docs page gives a dark board.
  const [canvasTheme, setCanvasTheme] = React.useState<'light' | 'dark'>(() => {
    if (typeof document === 'undefined') return 'light';
    try { const saved = JSON.parse(localStorage.getItem('flavor-theme') || 'null'); if (saved?.mode === 'dark' || saved?.mode === 'light') return saved.mode; } catch {}
    return document.documentElement.getAttribute('data-mode') === 'dark' || matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
  });
  /* [extension §7 — repo-file persistence] The reference keeps annotations in memory and
     token overrides in localStorage. Here both are ALSO written to .canvas/feedback.json
     through the dev server (canvas-vite-plugin.mjs) so feedback travels with the branch
     and the MCP can serve it (flavor_canvas_feedback). On the static build the endpoint
     is absent and this degrades to the reference's behaviour; the header pill says which. */
  const [persistMode, setPersistMode] = React.useState<'file' | 'local' | 'unknown'>('unknown');
  React.useEffect(() => {
    fetch(FEEDBACK_ENDPOINT).then((r) => (r.ok ? r.json() : Promise.reject())).then((d) => {
      setPersistMode('file');
      if (Array.isArray(d.comments) && d.comments.length) { setAnnotations(d.comments); idRef.current = Math.max(0, ...d.comments.map((c: Annotation) => c.id)); }
      if (d.overrides && Object.keys(d.overrides).length) setTokenOverrides(d.overrides);
    }).catch(() => setPersistMode('local'));
  }, []);
  const persistTimer = React.useRef<number | undefined>(undefined);
  React.useEffect(() => {
    if (persistMode !== 'file') return;
    clearTimeout(persistTimer.current);
    persistTimer.current = window.setTimeout(() => {
      fetch(FEEDBACK_ENDPOINT, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ version: 1, axes: { ...AXIS_DEFAULTS, mode: canvasTheme }, overrides: tokenOverrides, comments: annotations }) }).catch(() => {});
    }, 300);
  }, [annotations, tokenOverrides, canvasTheme, persistMode]);
  ...
  /* ── light / dark preview: apply .dark to the canvas + every frame ─────────
     The whole review surface (chrome, token editor, and every page iframe)
     flips together, so the token editor's swatches and picker read the active
     theme's values straight from live CSS with no extra plumbing.          */

  // Adapter §5 (theming bridge): Flavor resolves per theme AXES set as data-attributes on
  // <html>. The board runs under the brand defaults with only mode owned by the canvas —
  // the same mechanism the site itself uses, so chrome, token editor and frames agree.
  const applyCanvasTheme = React.useCallback((root: HTMLElement, theme: 'light' | 'dark') => {
    applyAxes(root.ownerDocument, { ...AXIS_DEFAULTS, mode: theme });
    root.style.colorScheme = theme;
  }, []);

  // Restore the site's own saved mode when leaving the canvas, so navigating back
  // doesn't strand the docs in whatever theme was last previewed here.
  React.useEffect(() => {
    const root = document.documentElement;
      ...
      try {
        ...
        if (!highlightEl) {
          ...
          Object.assign(highlightEl.style, {
            ...
            background: 'rgba(37,99,235,0.12)' /* raw: reference highlight — kept verbatim */,
            borderRadius: '2px',
            display: 'none',
          });
    ...
    <div className="flex h-screen flex-col bg-background text-foreground">
      ...
      <header className="brand-navy-surface flex h-14 flex-shrink-0 items-center justify-between border-b border-brand-teal/30 px-5 text-brand-navy-foreground">
        <div className="flex items-baseline gap-2 font-display">
          <span className="text-base font-bold tracking-tight font-display">Flavor DS</span>
        ...
        <div className="flex items-center gap-3 text-sm">
          ...
          {/* [extension §7] where feedback is being persisted */}
          <span
            className="text-[0.65rem] text-brand-navy-foreground/70"
            title={persistMode === 'file' ? 'Persisting to .canvas/feedback.json (repo file, via the dev server)' : persistMode === 'local' ? 'No dev server: token overrides persist to this browser; comments are in memory. Use Copy feedback.' : ''}
          >
            {persistMode === 'file' ? '● repo file' : persistMode === 'local' ? '○ browser only' : ''}
          </span>
    ...
    <div className="flex-1 space-y-3 overflow-y-auto p-4">
      ...
      <div className="space-y-2">
        {Object.entries(COLOR_TOKENS).map(([key, token]) => {
          ...
          const detail = alias ? `→ ${alias[1]}` : value ? hslToHex(parseHslTriplet(value) ?? { h: 0, s: 0, l: 0 }) : token.cssVar;
          return (
...
/* ── token color control ──────────────────────────────────────────────────
   ...
   color. Emits the design-system storage format (an HSL triplet, or a
   `var(--palette)` alias) so live-apply + persistence keep working as-is.  */
...
function TokenColorControl({
  ...
  React.useEffect(() => {
    ...
    setHslDraft(value && !VAR_ALIAS_RE.test(value.trim()) ? value : formatHslTriplet(hsl));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [syncKey, open]);
  ...
  const swatchBg = value ? value : `var(${cssVar})`;
        ...
        <Tabs defaultValue="picker">
          ...
          <TabsContent value="picker" className="mt-3 space-y-3">
            ...
            <div
              ...
              style={{
                background: `linear-gradient(to top, #000, transparent), linear-gradient(to right, #fff, hsl(${hsv.h} 100% 50%))` /* raw: the S/V square IS the colour space */,
              }}
            >
              <span
            ...
            <div
              ...
              style={{
                background:
                  'linear-gradient(to right, hsl(0 100% 50%), hsl(60 100% 50%), hsl(120 100% 50%), hsl(180 100% 50%), hsl(240 100% 50%), hsl(300 100% 50%), hsl(360 100% 50%))' /* raw: the hue slider IS the spectrum */,
              }}
            >
              <span
