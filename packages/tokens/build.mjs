 ...
 */
const FONT_PAIRS = {
  // Technical grotesque. Space Grotesk's Space Mono lineage gives the headline a machined
  // character that Inter, the strongest libre UI face, sets off cleanly.
  flavor: { label: 'Flavor', display: `'Space Grotesk', 'Helvetica Neue', system-ui, sans-serif`, text: `'Inter', system-ui, -apple-system, sans-serif`, google: ['Space Grotesk:wght@500;700', 'Inter:wght@400;500;600;700'] },
  ...
  // Expressive. The brand default. Syne is an experimental display family (400–800) that keeps
  // the retro-futurist voice the brand asked for, with the hierarchy Righteous could not
  // provide; Archivo is a neutral grotesque that stays out of its way.
  expressive: { was: 'Atomic Age', label: 'Expressive (default)', display: `'Syne', system-ui, sans-serif`, text: `'Archivo', system-ui, sans-serif`, google: ['Syne:wght@600;700;800', 'Archivo:wght@400;500;600;700'] },
};
// The pairing bare `:root` resolves to (no data-font). Also meta.defaults.font, so the docs,
// MCP and Figma sync all agree on which pair is the default.
const FONT_DEFAULT = 'expressive';
 ...
 *   data-font:     ${Object.keys(FONT_PAIRS).join(' | ')}   (default: ${FONT_DEFAULT})
 *   dir:           ltr | rtl
 */`);
...
{
  const lines = [':root {'];
  lines.push(`  --font-display: ${FONT_PAIRS[FONT_DEFAULT].display};`);
  lines.push(`  --font-text: ${FONT_PAIRS[FONT_DEFAULT].text};`);
...
const meta = {
  ...
  defaults: { hue: 'brand', sat: 'regular', mode: 'light', contrast: 'aa', bg: 'primary', radius: 'rounded', density: 'regular', font: FONT_DEFAULT, dir: 'ltr' },
